...
cdef class SegmentationNormal:
    ...
    def get_eps_angle(self):
        return (<pcl_seg.SACSegmentationFromNormals_t*>self.me).getEpsAngle()


    def set_axis(self, double ax1, double ax2, double ax3):
        cdef eigen3.Vector3f* vec = new eigen3.Vector3f(ax1, ax2, ax3)
        (<pcl_seg.SACSegmentationFromNormals_t*>self.me).setAxis(deref(vec))


    def get_axis(self):
        vec = (<pcl_seg.SACSegmentationFromNormals_t*>self.me).getAxis()
